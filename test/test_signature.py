...
@pytest.mark.skipif(sys.platform == "win32", reason="Windows executable cannot execute bash loop.")
def test_signature_after_removal_of_files(clear_signatures, clear_now_and_after):
...
def test_signature_with_dependency_tracing_and_vars(clear_signatures, clear_now_and_after):
    ...
    script = r"""
        ...
        input: 'out.txt'
        """
    res = execute_workflow(script, options={"trace_existing": True}, args=["--par", "10"])
    assert res["__completed__"]["__substep_completed__"] == 2
    # change parameter, the step should not be skipped
    res = res = execute_workflow(script, options={"trace_existing": True}, args=["--par", "20"])
    assert res["__completed__"]["__substep_completed__"] == 2
