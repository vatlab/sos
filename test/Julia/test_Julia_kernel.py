...
#dataframe
    ...
    def testPutJuliaDataToPython(self):
        with sos_kernel() as kc:
            ...
            #wait_for_idle(kc)
            execute(kc=kc, code="%put num_var num_arr_var logic_var logic_arr_var char_var char_arr_var mat_var recursive_var comp_var")
            wait_for_idle(kc)
            execute(kc=kc, code='''
%use sos
seri_var = list(seri_var)
...
#''')
#            wait_for_idle(kc)
            execute(kc=kc, code="%dict num_var num_arr_var logic_var logic_arr_var char_var char_arr_var mat_var recursive_var comp_var")
            res = get_result(iopub)
            #self.assertEqual(res['null_var'], None)
            self.assertEqual(res['num_var'], 123)
            self.assertEqual(list(res['num_arr_var']), [1,2,3])
