...
def test_command_line():
    ...
    assert subprocess.call("sos config -h", stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, shell=True) == 0
    assert subprocess.call("sos config -g --get", stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, shell=True) == 0
    assert subprocess.call("sos config", stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, shell=True) == 0
    assert subprocess.call("sos config --get", stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, shell=True) == 0
    assert (
        subprocess.call("sos config -g --set a 5", stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, shell=True)
        == 0
    )
    assert subprocess.call("sos config --get a", stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, shell=True) == 0
    assert (
        subprocess.call("sos config -g --unset a", stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, shell=True)
        == 0
    )
...
def test_config_set(config_factory):
    ...
    assert (
        subprocess.call(
            f"sos config --set cut 0.5 -c {myconfig}", stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, shell=True
        )
        == 0
    )
    ...
    assert (
        subprocess.call(
            f"sos config --set cut2 a3 -c {myconfig}", stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, shell=True
        )
        == 0
    )
    load_config_files(myconfig)
...
def test_get_config(config_factory):
    myconfig = config_factory(
        {
            "val": 5,
            "A": {"B.C": "33", "B.C1": {"D": "34"}, "D": "45"},
            "E": {"F": {"val": 6, "val1": 10, "G": "{val + val1}"}, "H": "{val}"},
            "O": "A{nonexisting}",
    ...
    assert get_config("E.F", val=7, val1=20) == {"val": 6, "val1": 10, "G": "27"}
    assert get_config("E.F", {"val": 8, "val1": 30}) == {"val": 6, "val1": 10, "G": "38"}
