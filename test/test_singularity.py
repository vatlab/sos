...
@pytest.mark.skipif(not shutil.which("singularity"), reason="Skip test because docker is not installed.")
...
@pytest.mark.skipif(
    not shutil.which("singularity") or sys.platform == "win32" or "TRAVIS" in os.environ or "APPVEYOR" in os.environ,
...
@pytest.mark.skipif(
    not shutil.which("singularity") or sys.platform == "win32", reason="Skip test because docker is not installed."
)
