    ...
    def _install(self, name, autoinstall):
        ...
        if spam_spec is not None:
            if self._version:
                ...
                if self._version.startswith("==") and metadata.parse_version(ver) == metadata.parse_version(
                    self._version[2:]
                ):
                    pass
                elif self._version.startswith("<=") and metadata.parse_version(ver) <= metadata.parse_version(
                    self._version[2:]
                ):
                    pass
                elif (
                    self._version.startswith("<")
                    and not self._version.startswith("<=")
                    and metadata.parse_version(ver) < metadata.parse_version(self._version[1:])
                ):
                    pass
                elif self._version.startswith(">=") and metadata.parse_version(ver) >= metadata.parse_version(
                    self._version[2:]
                ...
                # the case of >
                    ...
                    pass
                elif self._version[0] not in ("=", ">", "<", "!") and metadata.parse_version(
                    ver
                ) == metadata.parse_version(self._version):
                    pass
                else:
                    env.logger.warning(
