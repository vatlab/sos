...
class Visualizer:
    ...
    def _handle_table(self, df):
        ...
        code = (
            "".join(
                f"""{x} &nbsp; <i class="fa fa-sort" style="color:lightgray" onclick="sortDataFrame('{tid}', {idx}, '{index_type if idx == 0 else col_type[idx - 1]}')"></th>"""
                if "<th" in x
                else x
    ...
    def _natural_ticks(self, rg):
        ...
        if logh - logl < 3:
            return None
        return [10**x for x in range(logl, logh + 1)]
    ...
    def _handle_scatterplot(self, df):
        ...
        if args.by:
            ...
            for by_col in args.by:
                vals.append(sorted(set(data[by_col])))
            # outer product
            import itertools
        ...
        for col in args.cols[1:]:
            ...
            if args.by:
                for cat in categories:
                    ...
                    series["data"] = [
                        all_data[i] for i in range(len(all_data)) if all(data[b][i] == v for b, v in zip(args.by, cat))
                    ]
        ...
        options["series"]["lines"] = {
            "show": is_sorted(val_x) and not args.by if not args.show or "lines" in args.show else False
        }
