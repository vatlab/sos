...
class WorkflowSig:
    ...
    def convert_time(self, info):
        for key in [x for x in info.keys() if x.endswith("_time")]:
            info[key + "_str"] = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(info[key]))
    ...
    def transcripts(self):
        try:
            return {
                step: [self.convert_time(eval(x)) for x in items] for step, items in self.data["transcript"].items()
            }
        except Exception as e:
            env.logger.warning(str(e))
            return {}
