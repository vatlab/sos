...
class MasterTaskParams(TaskParams):
    def __init__(self, num_workers=0):
        ...
        self.global_def = ''
        self.task = ''
        self.sos_dict = {'_runtime': {}, '_input': [], '_output': [], '_depends': [], 'input': [], 'output':[], 'depends': [], 'step_name': '', '_index': 0}
        self.sigil = None
        ...
        for dirname, _, filelist in os.walk(os.path.join(os.path.expanduser('~'), '.sos', 'tasks')):
            for f in filelist:
                ...
                if ID in all_tasks:
                    ...
                    if workflows and os.path.splitext(f)[-1] in ('.def', '.task'):
                        task = loadTask(os.path.join(dirname, f))
                        if '__workflow_sig__' in task.sos_dict:
                            removable_ids.add(ID)
                    to_be_removed[ID].append(os.path.join(dirname, f))
        #
        for task in all_tasks:
            removed = True
            for f in to_be_removed[task]:
                if workflows and os.path.basename(f).split('.', 1)[0] not in removable_ids:
                    if verbosity > 2:
                        env.logger.debug('{} not removed because it is not generated by any workflow from the current project.'.format(f))
                    removed = False
                    continue
                try:
                    if verbosity > 3:
...
class TaskEngine(threading.Thread):
    ...
    def purge_tasks(self, tasks, purge_all=False, workflows=None, age=None, status=None, verbosity=2):
        try:
            return self.agent.check_output("sos purge {} {} {} {} {} -v {}".format(
                ' '.join(tasks), '--all' if purge_all else '',
                '--workflows {}'.format(' '.join(workflows)) if workflows else '',
