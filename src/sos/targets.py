...
class sos_targets(BaseTarget, Sequence, os.PathLike):
    ...
    def extend(self, another, source="", keep_groups=False):
        ...
        if arg._groups:
            ...
            if not self._groups:
                self._groups = [_sos_group(range(n_old), parent=self) for g in arg._groups]
            if len(self._groups) == 1 and len(arg._groups) > 1:
                # 1 vs more, we duplicate itself
                self._groups = [
                    _sos_group(self._groups[0]._indexes, self._groups[0]._labels).set(**self._groups[0]._dict)
    ...
    def select(self, i):
        ...
        if callable(i):
            ...
            if not self._groups:
                return ret
            index_map = {o_idx: n_idx for n_idx, o_idx in zip(range(len(ret._targets)), kept)}
            kept = set(kept)
            for grp in self._groups:
                ret._groups.append(
                    _sos_group(
    ...
    def paired_with(self, name, properties):
        ...
        if isinstance(properties, (bool, int, float, str, bytes)):
            ...
            raise ValueError(f"Unacceptable properties {properties} for function paired_with")
        return self
    ...
    def remove_targets(self, type, kept=None):
        ...
        if not self._groups:
            return self
        index_map = {o_idx: n_idx for n_idx, o_idx in zip(range(len(self._targets)), kept)}
        kept = set(kept)
    ...
    def _add_groups(self, grps):
        ...
        for grp in grps:
            ...
            self._groups.append(_sos_group(range(start_idx, start_idx + grp_size), labels=grp._labels).set(**grp._dict))
    ...
    def _duplicate_groups(self, n):
        ...
        for _ in range(n - 1):
            for grp in self._groups[:n_grps]:
                self._groups.append(_sos_group(grp._indexes, grp._labels).set(**grp._dict))
        return self
    ...
    def _group(self, by):
        ...
        if by == "single":
            ...
            if len(self) % 2 != 0:
                raise ValueError(f"Paired by has to have even number of input files: {len(self)} provided")
            if by == "pairs":
                grp_size = 1
            else:
                try:
            ...
            labels = list(dict.fromkeys(self.labels))
            self._groups = [
                _sos_group([i for i, x in enumerate(self._labels) if x == src], parent=self) for src in labels
            ]
                    ...
                    if isinstance(grp, Sequence) and all(isinstance(x, int) for x in grp):
                        ...
                        for x in sos_targets(grp):
                            try:
                                ...
                                raise ValueError(f"Returned target is not one of the targets. {x}") from e
                        self._groups.append(_sos_group(index, parent=self))
            except Exception as e:
                raise ValueError(f"Failed to apply customized grouping method: {e}") from e
        else:
    ...
    def _handle_paired_with(self, paired_with):
        ...
        if paired_with is None or not paired_with:
            ...
            try:
                ...
                raise ValueError(f"Invalid value for option paired_with {paired_with}") from e
            var_value = []
            for vn in var_name:
    ...
    def _handle_group_with(self, group_with):
        ...
        if group_with is None or not group_with:
            ...
            try:
                ...
                raise ValueError(f"Invalud value for option group_with {group_with}") from e
            var_value = []
            for vn in var_name:
                if vn[1:] not in env.sos_dict:
                    raise ValueError(f"Variable {vn[1:]} does not exist.")
            ...
            for name, values in zip(fe_iter_names, fe_values):
                if not isinstance(values, Sequence):
                    try:
                        ...
                        raise ValueError(f"Cannot iterate through variable {name}: {e}") from e
    ...
    def write(self):
        ...
        for f in self.input_files:
            try:
                ...
                env.logger.debug(f"Failed to create signature: input target {f} does not exist")
                return False
        output_sig = {}
        for f in self.output_files:
            try:
                ...
                env.logger.debug(f"Failed to create signature: output target {f} does not exist")
                return False
        dependent_sig = {}
        for f in self.dependent_files:
            try:
                ...
                env.logger.debug(f"Failed to create signature: dependent target {f} does not exist")
                return False
        init_context_sig = {
...
class RuntimeInfo(InMemorySignature):
    ...
    def release(self, quiet=False):
        ...
        if not hasattr(self, "_lock") or self._lock is None:
            env.logger.warning(f"Releasing an non-existent or released lock for {self.sig_id}.")
            return
        if self._lock:
            try:
                self._lock.release()
