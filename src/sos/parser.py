...
from .utils import Error, as_fstring, env, format_par, locate_script, separate_options, textMD5, valid_expr_till
...
def is_type_hint(stmt: str) -> bool:
    ...
    if _action_list is None:
        ...
        _action_list = [x.name for x in metadata.entry_points(group="sos_actions")]
        ...
        for field in args:
            if field.arg == optname:
                try:
                    ...
                    raise ValueError(f"A constant value is expected for option {optname}: {options} provided.") from e
        return default_value, options
    except SyntaxError as e:
        raise ValueError(f"Expect a list of keyword arguments: {options} provided") from e
...
class SoS_Step:
    ...
    def finalize(self) -> None:
        ...
        # handle tasks
        input_directive = [
            idx for idx, statement in enumerate(self.statements) if statement[0] == ":" and statement[1] == "input"
        ]
        task_directive = [
            idx for idx, statement in enumerate(self.statements) if statement[0] == ":" and statement[1] == "task"
        ]
        ...
        if (
            ...
            and len([x for x in self.statements if x[0] == ":" and x[1] == "output"]) == 1
        ):
    ...
    def show(self):
        ...
        print("\n".join(textwrap.wrap(text, width=textWidth, initial_indent="", subsequent_indent=" " * 24)))
        local_parameters = {x: y for x, y in self.parameters.items() if x not in self.global_parameters}
        if local_parameters:
...
class SoS_Script:
    def __init__(self, content: Optional[str] = "", filename: Optional[str] = None) -> None:
        ...
        forward_section_steps = sum([x.names for x in self.sections if "provides" not in x.options], [])
        # (name, None) is auxiliary steps
        self.workflows = list(dict.fromkeys([x[0] for x in all_section_steps if "*" not in x[0]]))
        forward_workflows = list(dict.fromkeys([x[0] for x in forward_section_steps if "*" not in x[0]]))
            ...
            if mo:
                ...
                for name in section_name.split(","):
                    ...
                    if mo:
                        ...
                        if n == "":
                            parsing_errors.append(lineno, line, "Empty step name is not allowed")
                    ...
                    try:
                        ...
                        for option in pieces:
                            ...
                            if mo:
                                ...
                                parsing_errors.append(lineno, line, f'Invalid section option "{option}"')
                    except Exception as e:
                        parsing_errors.append(lineno, line, str(e))
                    if "EXECUTOR" in env.config["SOS_DEBUG"] or "ALL" in env.config["SOS_DEBUG"]:
                        env.log_to_file("EXECUTOR", f"Header parsed with names {step_names} and options {step_options}")
                for name in step_names:
                    ...
                    for prev_name in all_step_names:
                        ...
                        if len(set(prev_names) & set(names)) > 0 and "global" not in names:
