...
class Base_Step_Executor:
    ...
    def execute(self, stmt, return_result=False):
        try:
            # if there are concurrent subworkflow, the workflow is executed
            # before the step, so the output might be generated by the subworkflow.
            if not env.sos_dict.get("__concurrent_subworkflow__", False):
                clear_output()
