...
class TaskEngine(threading.Thread):
    ...
    def notify_controller(self, msg):
        if env.config["exec_mode"]:
            ...
            env.tapping_listener_socket.send(encode_msg({"msg_type": "task_status", "data": msg}))
    ...
    def monitor_tasks(self, tasks=None, status=None, age=None):
        ...
        return sorted(
            [
                (x, self.task_status[x], self.task_info[x].get("data", (time.time(), None, None)))
                for x in tasks
                if (status is None or self.task_status[x] in status)
                and (
                    age is None
                    or (
    ...
    def run(self):
        ...
        status_output = self.query_tasks(check_all=True, verbosity=3, numeric_times=True)
        with threading.Lock():
        ...
        while True:
            ...
            if (
                self.running_tasks or self.running_pending_tasks or self.pending_tasks
            ) and time.time() - self._last_status_check > self.status_check_interval:
                if self._status_checker is None:
                    self._status_checker = self._thread_workers.submit(
                        self.query_tasks,
                        self.running_tasks + list(self.running_pending_tasks.keys()),
                        check_all=False,
                        verbosity=3,
                        numeric_times=True,
                    )
                ...
                for line in status_output.splitlines():
                    ...
                    try:
                        ...
                        if tid not in self.running_tasks + list(self.running_pending_tasks.keys()):
                            ...
                            # are overwhelmed
                            env.log_to_file("TASK", f"Task {tid} removed since status check.")
                            continue
                        self.task_info[tid]["date"] = [
                            float(ct) if ct.strip() else None,
                            float(st) if st.strip() else None,
            ...
            if self.pending_tasks:
                num_active_tasks = sum(len(x) for x in self.submitting_tasks.keys()) + len(self.running_tasks)
                ...
                with threading.Lock():
                    for idx, tid in enumerate(self.pending_tasks):
                        ...
                        if (
                            len(slot) == self.batch_size
                            or idx == len(self.pending_tasks) - 1
                ...
                for slot in slots:
                    ...
                    for s_tid in slot:
                        env.log_to_file(
                            "TASK", f"Start submitting {s_tid} (status: {self.task_status.get(s_tid, 'unknown')})"
                        )
                ...
                if time.time() - self.last_report > 60:
                    self.last_report = time.time()
                    env.log_to_file("TASK", "No running or pending task. Task engine is idle.")
        ...
        with lock:
            ...
            if task_id in self.task_status and self.task_status[task_id]:
                ...
                if self.task_status[task_id] == "completed":
                    ...
                    env.logger.info(f"{task_id} ``restart`` from status ``{self.task_status[task_id]}``")
        ...
        with threading.Lock():
            ...
            if status == "missing":
                ...
                if task_id in self.task_status and self.task_status[task_id] == status:
                    ...
                    if status == "running":
                        if task_id not in self.task_info:
                            self.task_info[task_id]["date"] = [time.time(), time.time(), 0]
            ...
            if status == "new":
                if task_id not in self.pending_tasks:
                    ...
                    env.logger.warning('Task in "new" status when it is supposed to be running. Resubmitting.')
            ...
            if status in ("completed", "failed", "aborted") and task_id in self.running_tasks:
                ...
                self.task_results[task_id] = self._thread_workers.submit(self.agent.receive_result, task_id)
    ...
    def kill_tasks(self, tasks, tags=None, all_tasks=False):
        ...
        try:
            ...
            env.logger.error("Failed to kill all tasks" if all_tasks else f"Failed to kill tasks {' '.join(tasks)}")
            return ""
        return ret
...
class BackgroundProcess_TaskEngine(TaskEngine):
    ...
    def _submit_task_with_template(self, task_ids):
        ...
        for task_id in task_ids:
            ...
            task_runtime["_runtime"].update(tf.params.sos_dict.get("_runtime", {}))

            runtime["command"] = (
                f"{self.config.get('sos', 'sos')} execute {task_id} -v {env.verbosity} -s {env.config.get('sig_mode', 'default')} -m {env.config.get('run_mode', 'run')}"
            )
            ...
            try:
                ...
                raise ValueError(f"Failed to generate job file for task {task_id}: {e}") from e

        filename = task_ids[0] + (".sh" if len(task_ids) == 1 else f"-{task_ids[-1]}.sh")
        # now we need to write a job file
        job_file = os.path.join(os.path.expanduser("~"), ".sos", "tasks", filename)
